...
def evaluate_summary(machine_summary, user_summary, eval_metric='avg'):
    """ Dato che il nostro user summary è già binario ed è solo 1, andiamo
        ad estrarre l'unico e lo poniamo su un array. Dopodiché utilizziamo
        una soglia al di sopra del cui il machine summary è considerato 1, e
        calcoliamo l'F-score tra le due summarization.
    """    
    THRESHOLD = 0.4
    machine_summary = np.array(machine_summary > THRESHOLD, dtype=int)    
    user_summary = np.array(user_summary[0][:len(machine_summary)], dtype=int)
    f1 = f1_score(user_summary, machine_summary)
    re = recall_score(user_summary, machine_summary)
    pr = precision_score(user_summary, machine_summary)
    return f1, re, pr
